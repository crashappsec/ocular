...
build-helm: kubebuilder helmpatch-plugin yq ## Generate a helm-chart using kubebuilder
	...
	@"$(YQ)" -ie '.manager.labels = {}' dist/chart/values.yaml
	@"$(YQ)" -ie '.manager.annotations = {}' dist/chart/values.yaml
	@"$(YQ)" -ie '.manager.image = {"repository": strenv(OCULAR_CONTROLLER_REPOSITORY), "pullPolicy": "IfNotPresent", "tag": strenv(OCULAR_VERSION)}' dist/chart/values.yaml
	@"$(YQ)" -ie '.sidecar.image =  {"repository": strenv(OCULAR_SIDECAR_REPOSITORY), "pullPolicy": "IfNotPresent", "tag": strenv(OCULAR_VERSION)}' dist/chart/values.yaml
	@"$(YQ)" -ie '(.sidecar | key) head_comment="Configure Ocular sidecar image"' dist/chart/values.yaml
