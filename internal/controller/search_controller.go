...
func (r *SearchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if search.Spec.ServiceAccountName == "" {
		...
		err = patchResource(ctx, r.Client, search, patch)
		return ctrl.Result{}, err
	}
	...
	if serviceAccountOp == controllerutil.OperationResultCreated ||
		...
		l.Info("serivce account was modified", "op", serviceAccountOp)
	}

	roleBinding := &rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: searchResourcePrefix + search.GetName(), Namespace: search.Namespace}}
	...
	if roleBindingOp == controllerutil.OperationResultCreated ||
		...
		l.Info("role binding was modified", "op", roleBindingOp)
	}

	searchPod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: searchResourcePrefix + search.GetName(), Namespace: search.Namespace}}
...
func (r *SearchReconciler) populateServiceAccount(search *v1beta1.Search, sa *corev1.ServiceAccount) error {
	...
	if sa.CreationTimestamp.IsZero() {
		...
		sa.Labels[v1beta1.TypeLabelKey] = v1beta1.SearchServiceAccountType
...
func (r *SearchReconciler) populateRoleBinding(search *v1beta1.Search, sa *corev1.ServiceAccount, rb *rbacv1.RoleBinding) error {
	...
	rb.Labels[v1beta1.TypeLabelKey] = v1beta1.SearchRoleBindingType
...
func (r *SearchReconciler) populateSearchPod(search *v1beta1.Search, pod *corev1.Pod, crawler resources.Invocation[v1beta1.CrawlerSpec]) error {
	...
	if pod.CreationTimestamp.IsZero() {
		...
		templateVolume := corev1.Volume{
			Name: searchTemplatesVolumeName,
			VolumeSource: corev1.VolumeSource{
				DownwardAPI: &corev1.DownwardAPIVolumeSource{
					Items: []corev1.DownwardAPIVolumeFile{{
						Path: pipelineTemplateName,
		...
		fifoVolume := corev1.Volume{
			Name:         searchFIFOVolumeName,
		...
		containerOpts := make([]containers.Option, 0, 5)
		containerOpts = append(containerOpts,
			...
			containers.WithAdditionalVolumeMounts(
				corev1.VolumeMount{
					Name:      templateVolume.Name,
					MountPath: configDirectory,
				}, corev1.VolumeMount{
					Name:      fifoVolume.Name,
					MountPath: fifoDirectory,
				}, corev1.VolumeMount{
		...
		pod.Labels[v1beta1.TypeLabelKey] = v1beta1.SearchPodType
		...
		schedulerContainer := containers.ApplyOptionsTo(corev1.Container{
			Name:            schedulerInitContainerName,
			Image:           r.SidecarImage,
			ImagePullPolicy: r.SidecarPullPolicy,
			Args:            []string{"init"},
			Resources:       *schedulerInitResourceRequirements.DeepCopy(),
			SecurityContext: &corev1.SecurityContext{
				RunAsNonRoot: new(true),
			},
		}, containerOpts...)

		pod.Spec.InitContainers = containers.ApplyStandardOptions([]corev1.Container{schedulerContainer})

		/* crawler container */
		crawlerOptions := append(containerOpts,
			containers.WithNamePrefix(crawlerContainerPrefix),
			containers.WrapCommand(schedulerBinaryPath, "crawler"),
			containers.WithAdditionalEnvVars(
				containers.ParseParameterEnvVars(crawler.Spec.Parameters, crawler.Parameters, nil)...,
			),
		)

		pod.Spec.Containers = containers.ApplyStandardOptions([]corev1.Container{
			containers.ApplyOptionsTo(crawlerContainer, crawlerOptions...)})

		pod.Spec.Volumes = append(crawler.Spec.Volumes, templateVolume, fifoVolume, binaryVolume)
...
func generateBaseSearchEnvironment(search *v1beta1.Search) []corev1.EnvVar {
	...
	return []corev1.EnvVar{
		...
		{
			Name:  v1beta1.EnvVarSchedulerPath,
			Value: schedulerBinaryPath,
		},
	}
}
