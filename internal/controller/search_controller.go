...
const (
	...
	pipelineFifoDir   = "/run/ocular"
	pipelineFifotFile = "targets.fifo"
	pipelineFifoPath  = pipelineFifoDir + "/" + pipelineFifotFile
)
...
func (r *SearchReconciler) newSearchPod(search *v1beta1.Search, crawlerSpec v1beta1.CrawlerSpec, sa *corev1.ServiceAccount, containerOpts ...containers.Option) (*corev1.Pod, error) {
	...
	awaitSidecarContainer := r.generateAwaitSidecarContainer(search)
	...
	pod := &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			...
			InitContainers:     containers.ApplyOptions([]corev1.Container{schedulerSidecarContainer}, containerOpts...),
			Containers:         containers.ApplyOptions([]corev1.Container{crawlerSpec.Container, awaitSidecarContainer}, containerOpts...),
...
const (
	...
	// sidecarAwaitContainerName is the name of the container
	// that keeps the pod alive until the scheduler is complete
	sidecarAwaitContainerName = "scheduler-await"
)
		...
		ReadinessProbe: &corev1.Probe{
			ProbeHandler: corev1.ProbeHandler{
				Exec: &corev1.ExecAction{
					Command: []string{"/entrypoint", "scheduler-ready"},
				},
			},
...
func (r *SearchReconciler) generateAwaitSidecarContainer(_ *v1beta1.Search) corev1.Container {
	var sidecarEnvVars []corev1.EnvVar

	return corev1.Container{
		Name:            sidecarAwaitContainerName,
		Image:           r.SidecarImage,
		ImagePullPolicy: r.SidecarPullPolicy,
		Args:            []string{"scheduler-await"},
		Env:             sidecarEnvVars,
	}
}
