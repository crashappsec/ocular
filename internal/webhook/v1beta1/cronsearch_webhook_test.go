...
var _ = Describe("CronSearch Webhook", func() {
	var (
		obj       *v1beta1.CronSearch
		oldObj    *v1beta1.CronSearch
		validator CronSearchCustomValidator
		defaulter CronSearchCustomDefaulter
	)

	const validCronSearchName = "valid-cronsearch-name"
	const schedule = "*/5 * * * *"
	...
	Context("When creating CronSearch under Defaulting Webhook", func() {
		It("Should apply defaults when a required field is empty", func() {
			...
			Expect(obj.Spec.ConcurrencyPolicy).To(Equal(v1beta1.AllowConcurrent), "Expected ConcurrencyPolicy to default to AllowConcurrent")
		...
		It("Should not overwrite fields that are already set", func() {
			...
			Expect(obj.Spec.ConcurrencyPolicy).To(Equal(v1beta1.ForbidConcurrent), "Expected ConcurrencyPolicy to retain its set value")
	...
	Context("When creating or updating CronSearch under Validating Webhook", func() {
		...
		It("Should deny update if both name and spec are invalid", func() {
			oldObj.Name = validCronSearchName
		...
		It("Should admit update if both name and spec are valid", func() {
			...
			By("simulating an update")
			obj.Name = "valid-cronsearch-name-updated"
